...
We increase supervision frequency ($K{=}5$ vs.\ $K{=}20$) and tighten the tolerance floor ($\tau_{\min}{=}0.003$) to prioritize quality over acceptance rate. With $K{=}5$, the draft model sees 4$\times$ more real gradient data and rejects speculative updates more aggressively. Critically, we also replace the EMA baseline tracker with direct assignment so the verifier's reference loss always reflects the most recent supervision step, eliminating baseline drift.

\begin{table}[h]
\centering
\caption{Quality-Focused Configuration ($K{=}5$, $\tau_{\min}{=}0.003$)}
...
The quality-focused configuration successfully reduces degradation from 12.5\% to \textbf{4.6\%}---well under the 5\% target---by maintaining a stable 57--58\% acceptance rate throughout training. The acceptance rate starts high ($\sim$90\%) during warmup and settles to $\sim$58\% by step 500, remaining stable thereafter. This confirms that the direct baseline assignment and tighter tolerance create a genuine quality gate rather than the rubber-stamping behavior observed with the original EMA-based verifier.
...
The original configuration exhibited 12.5\% loss degradation due to verifier baseline drift (the EMA tracker lagged behind actual loss, making the acceptance threshold permissive). The quality-focused configuration (Section~\ref{sec:ablations}) reduces this to \textbf{4.6\%} via three fixes: (1) direct baseline assignment instead of EMA, (2) tighter tolerance floor ($\tau_{\min}{=}0.003$), and (3) lowered target acceptance ceiling (70\%). Further mitigations include:

\begin{enumerate}
    \item \textbf{Hybrid training:} Use LST for the first 80\% of steps (large speedup) and switch to standard training for the final 20\% (loss recovery to $\sim$0\%).
...
\begin{enumerate}
    ...
    \item \textbf{Loss quality gap:} The original 12.5\% loss degradation has been reduced to 4.6\% via the quality-focused configuration. The hybrid schedule (Section~\ref{sec:ablations}) is expected to close the remaining gap to $\sim$0\%.
